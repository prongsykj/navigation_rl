...
class ActorCritic:
	...
	def create_actor_model(self):
		...
		picture1 = tf.reshape(obs_history[:,:,:],[-1,self.obs_dim])
        # 2 dimension to (batch*trace_len,32,32,3)
		picture1 = tf.reshape(picture1[:,:picture_listnum],[-1,picture_shape[0], 
                                                                picture_shape[1], 
                                                                picture_shape[2]])
		# picture = tf.expand_dims(picture,axis=0)
		c11 = Conv2D(16,2,activation='relu',strides=(2, 2))(picture1)
		c11 = BatchNormalization()(c11)
		m11 = MaxPooling2D(pool_size=(2, 2),strides=(2, 2))(c11)
		c12 = Conv2D(32,2,activation='tanh',strides=(2, 2))(m11)
		c12 = BatchNormalization()(c12)
		m12 = MaxPooling2D(pool_size=(2, 2),strides=(2, 2))(c12)
		m12 = tf.reshape(m12,[-1,obs_history.shape[1],m12.shape[1]*m12.shape[2]*m12.shape[3]])
		concat_obs = Concatenate()([m12,obs_history[:,:,picture_listnum:],action_history])
		lstm_out = LSTM(512)(concat_obs)

		#########################  the section of now state  #########################
        # 3 dimension to 2 dimension
		picture2 = tf.reshape(obs_t[:,:],[-1,self.obs_dim])
        # 2 dimension to (batch,32,32,3)
		picture2 = tf.reshape(picture2[:,:picture_listnum],[-1,picture_shape[0], 
                                                                picture_shape[1], 
                                                                picture_shape[2]])
		c21 = Conv2D(16,2,activation='relu',strides=(2, 2))(picture2)
		c21 = BatchNormalization()(c21)
		m21 = MaxPooling2D(pool_size=(2, 2),strides=(2, 2))(c21)
		c22 = Conv2D(32,2,activation='tanh',strides=(2, 2))(m21)
		c22 = BatchNormalization()(c22)
		m22 = MaxPooling2D(pool_size=(2, 2),strides=(2, 2))(c22)
		m22 = tf.reshape(m22,[-1,m22.shape[1]*m22.shape[2]*m22.shape[3]])
		concat_now = Concatenate()([m22,obs_t[:,picture_listnum:]])
	...
	def create_critic_model(self):
		...
		m12 = MaxPooling2D(pool_size=(2, 2),strides=(2, 2))(c12)
		m12 = tf.reshape(m12,[-1,obs_history.shape[1],m12.shape[1]*m12.shape[2]*m12.shape[3]])
		concat_history = Concatenate()([m12,obs_history[:,:,picture_listnum:],action_history])
		...
		m22 = MaxPooling2D(pool_size=(2, 2),strides=(2, 2))(c22)
		m22 = tf.reshape(m22,[-1,m22.shape[1]*m22.shape[2]*m22.shape[3]])
		concat_now = Concatenate()([m22,obs_t[:,picture_listnum:],action_t])
		d21 = Dense(512, activation='relu')(concat_now)
...
def main():
	...
	# actor_critic.actor_model.load_weights("actormodel-24-1200.h5")
	# actor_critic.critic_model.load_weights("criticmodel-24-1200.h5")
	for i in range(num_trials):
		...
		for j in range(trial_len):
			...
			##########################################################################################
		np.save('reward_camera_12.2/reward_epoch_{}'.format(i), total_reward)
