...
from keras.layers import Dense, Dropout, Input, merge,Conv2D,MaxPooling2D,BatchNormalization
...
class ActorCritic:
	...
	def create_actor_model(self):
		state_input = Input(shape=sequence*picture_listnum + 28)
		picture = state_input[:,:sequence*picture_listnum]
		picture = tf.reshape(picture,[-1,picture_shape[0],picture_shape[1],picture_shape[2]*sequence])
		# picture = tf.expand_dims(picture,axis=0)
		c1 = Conv2D(16,2,activation='relu',strides=(2, 2))(picture)
		m1 = MaxPooling2D(pool_size=(2, 2),strides=(2, 2))(c1)
		c2 = Conv2D(32,2,activation='tanh',strides=(2, 2))(m1)
		m2 = MaxPooling2D(pool_size=(2, 2),strides=(2, 2))(c2)
		m2 = tf.reshape(m2,[-1,m2.shape[1]*m2.shape[2]*m2.shape[3]])
		concat_all = Concatenate()([m2,state_input[:,sequence*picture_listnum:]])
		h1 = Dense(512, activation='relu')(concat_all)
		h2 = Dense(512, activation='relu')(h1)
		h2 = Concatenate()([h2,state_input[:,sequence*picture_listnum+24:]])
	...
	def create_critic_model(self):
		...
		m2 = tf.reshape(m2,[-1,m2.shape[1]*m2.shape[2]*m2.shape[3]])
		concat_all = Concatenate()([m2,state_input[:,sequence*picture_listnum:]])
		state_h1 = Dense(512, activation='relu')(concat_all)
		...
		merged_h1 = Concatenate()([merged_h1,state_input[:,sequence*picture_listnum+24:]])
	...
	def act(self, cur_state):  # this function returns action, which is predicted by the model. parameter is epsilon
		...
		action[0][0] = action[0][0] + (np.random.random()-0.5)*0.1
		action[0][1] = action[0][1] + np.random.random()*0.2
		return action	
...
def main():
	...
	########################################################
	game_state= ddpg_LSTM_camera.GameState()   # game_state has frame_step(action) function
	...
	if (train_indicator==2):
		for i in range(num_trials):
			...
			for j in range(100):
				...
				reward, new_state, crashed_value = game_state.game_step(0.1, action[0][1]*5, action[0][0]*5) 
	...
	if (train_indicator==1):
		...
		actor_critic.actor_model.load_weights("actormodel-4-1200.h5")
		actor_critic.critic_model.load_weights("criticmodel-4-1200.h5")
		for i in range(num_trials):
			#print("trial:" + str(i))
			#game_state.game_step(0.3, 0.2, 0.0)
			#game_state.reset()

			current_state = game_state.reset()[:picture_listnum]
			for nums in range(sequence-2):
					current_state = np.append(current_state,game_state.reset()[:picture_listnum])
			current_state = np.append(current_state,game_state.reset())
			##############################################################################################
			total_reward = 0
			...
			for j in range(trial_len):
				...
				###########################################################################################
				#print('wanted value is %s:', game_state.observation_space.shape[0])
				current_state = current_state.reshape((1, picture_listnum*sequence+28))
				action = actor_critic.act(current_state)
				action = action.reshape((1, game_state.action_space.shape[0]))
				print("action is speed: ",action[0][1]," angular: ",action[0][0])


                # save six pictures  sequence = 6
				reward, new_state, crashed_value = game_state.game_step(0.1, action[0][1], action[0][0])
				sum_new_state = new_state[:,:picture_listnum]
				for nums in range(sequence-2):
					reward, new_state, crashed_value = game_state.game_step(0.1, action[0][1], action[0][0])
					sum_new_state = np.append(sum_new_state,new_state[:,:picture_listnum])
				reward, new_state, crashed_value = game_state.game_step(0.1, action[0][1], action[0][0])
				sum_new_state = np.append(sum_new_state,new_state)

				total_reward = total_reward + reward
				###########################################################################################
				# picture = new_state[:,:picture_listnum]
				# picture = np.array(picture)
				# picture = picture.reshape(picture_shape[0],picture_shape[1],picture_shape[2])
				...
				sum_new_state = sum_new_state.reshape((1, picture_listnum*sequence+28))

				# print shape of current_state
				#print("current_state is %s", current_state)
				##########################################################################################
				actor_critic.remember(current_state, action, reward, sum_new_state, crashed_value)
				current_state = sum_new_state
