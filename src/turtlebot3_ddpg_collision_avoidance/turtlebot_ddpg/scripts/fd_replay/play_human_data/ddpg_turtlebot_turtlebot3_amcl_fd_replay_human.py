...
class GameState:
    ...
    def __init__(self):
        ...
        self.last_normalized_laser = [0]*24
    ...
    def reset(self):
        ...
        # self.target_x = 7+4*(np.random.random()-0.5)         #5到9之间
        # self.target_y = -6
    ...
    def turtlebot_is_crashed(self, laser_values, range_limit):
        ...
        for i in range(len(laser_values)):
            if (laser_values[i] < 2.5*range_limit):
                self.laser_crashed_reward = -150
            if (laser_values[i] < range_limit):
                self.laser_crashed_value = 1
                self.laser_crashed_reward = -400
                self.reset()
                time.sleep(1)
                break
    ...
    def game_step(self, time_step=0.1, linear_x=0.8, angular_z=0.3):
        ...
        record_time_step = 0
        self.move_cmd.linear.x = linear_x*0.3
        ...
        #print("linear_x*0.26 is %s", linear_x*0.26)
        ...
        np.save('trace_laser/trace_laser_x2/tracex_epoch_{}'.format(self.tra_num), turtlebot_x)
        np.save('trace_laser/trace_laser_y2/tracey_epoch_{}'.format(self.tra_num), turtlebot_y)
        self.tra_num += 1
        ...
        # #如果当前距离比历史上任何最短距离都小，增加奖励值
        # if(distance_turtlebot_target<self.nearest_distance):
        #     distance_reward = distance_turtlebot_target-self.nearest_distance
        #     self.nearest_distance = distance_turtlebot_target
        # else:
        #     distance_reward = 0

        #如果上次最短距离比这次最短距离还要长，奖励值减小
        now_distance = min(normalized_laser)
        last_distance = min(self.last_normalized_laser)
        print(now_distance - last_distance)
        if last_distance < 0.8 and (now_distance - last_distance) < 0:
            distance_reward -= 0
        self.last_normalized_laser = normalized_laser.copy()
        ...
        reward  = distance_reward*(5/time_step)*15 + self.arrive_reward + self.collision_reward + self.angular_punish_reward + self.linear_punish_reward
        ...
        print("distance reward is : %s", distance_reward*(5/time_step)*15)
        print(1111111111111111111111111111111111111111111)
