...
class ActorCritic:
	...
	def create_actor_model(self):
		...
		# picture = state_input[:,:picture_listnum]
		# picture = tf.reshape(picture,[-1,picture_shape[0],picture_shape[1],picture_shape[2]])
		...
		# concat_all = Concatenate()([m2,state_input[:,picture_listnum:]])
		h1 = Dense(512, activation='relu')(state_input[:,picture_listnum:])
		#h1 = Concatenate()([h1,state_input[:,picture_listnum+24:]])
		h2 = Dense(512, activation='relu')(h1)
		#h2 = Concatenate()([h2,state_input[:,picture_listnum+24:]])
		h3 = Dense(512, activation='relu')(h2)
		#h3 = Concatenate()([h3,state_input[:,picture_listnum+24:]])
	...
	def create_critic_model(self):
		...
		# m2 = tf.reshape(m2,[-1,m2.shape[1]*m2.shape[2]*m2.shape[3]])
		# concat_all = Concatenate()([m2,state_input[:,picture_listnum:]])
		state_h1 = Dense(512, activation='relu')(state_input[:,picture_listnum:])

		action_input = Input(shape=self.env.action_space.shape)
		action_h1    = Dense(512)(action_input)

		merged    = Concatenate()([state_h1, action_h1,state_input[:,picture_listnum+24:]])
		merged    = Concatenate()([state_h1, action_h1])
		merged_h1 = Dense(512, activation='relu')(merged)
		#merged_h1 = Concatenate()([merged_h1,state_input[:,picture_listnum+24:]])
		merged_h2 = Dense(512, activation='relu')(merged_h1)
		#merged_h2 = Concatenate()([merged_h2,state_input[:,picture_listnum+24:]])
...
def main():
	...
	if (train_indicator==1):
		...
		# actor_critic.actor_model.load_weights("actormodel-22-200.h5")
		# actor_critic.critic_model.load_weights("criticmodel-22-200.h5")
		for i in range(num_trials):
			#print("trial:" + str(i))
			#game_state.game_step(0.3, 0.2, 0.0)
			#game_state.reset()
			...
			for j in range(trial_len):
				...
				##########################################################################################
			np.save('reward_only_laser/reward_epoch_{}'.format(i), total_reward)
